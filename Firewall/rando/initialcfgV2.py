...
class FW_IPTables:
    ...
    def main_forward_set(self):
        ...
        if (EXTERNALDNS == True):        
            subprocess.call(['sudo', 'iptables', '-A', 'FORWARD', '-p', 'udp', '--destination-port', '53', '-j', 'REJECT'])
            subprocess.call(['sudo', 'iptables', '-A', 'FORWARD', '-p', 'tcp', '--destination-port', '53', '-j', 'REJECT'])
        elif (EXTERNALDNS == False): 
            for chain in self.cChains:       
                subprocess.call(['sudo', 'iptables', '-A', 'FORWARD', '-p', 'udp', '--destination-port', '53', '-j', chain])
    ...
    def main_input_set(self):
        ...
        for chain in self.cChains:
            subprocess.call(['sudo', 'iptables', '-A', 'INPUT', '-p', 'udp', '--destination-port', '53', '-j', chain])

        subprocess.call(['sudo', 'iptables', '-A', 'INPUT', '-i', INIFACE, '-p', 'udp', '--destination-port', '53', '-j', 'ACCEPT'])
        subprocess.call(['sudo', 'iptables', '-A', 'INPUT', '-i', INIFACE, '-p', 'tcp', '--destination-port', '443', '-j', 'ACCEPT'])
    ...
    def NAT(self):
        nChains = ['PREROUTING', 'POSTROUTING', 'INPUT', 'OUTPUT']
        for chain in nChains:
            subprocess.call(['sudo', 'iptables', '-t', 'nat', '-P', chain, 'DROP'])
